...
export const graphql = (path: string): OakRouter =>
  (new OakRouter()).post(
    path,
...
export const rest = (path: string): OakRouter =>
  (new OakRouter()).post(path, Handler, CreateController).get(
    path,
    Handler,
    ReadController,
  ).get(`${path}/:id`, Handler, ReadController).put(
    `${path}/:id`,
    Handler,
    UpdateController,
  ).patch(`${path}/:id`, Handler, UpdateController).delete(
    `${path}/:id`,
    Handler,
    DeleteController,
  );

export const notFound = (router: OakRouter) =>
  router.all(
    "(.*)",
    async (ctx: Context): Promise<void> => {
...
export const make = (
  { path, type }: Pick<Config, "path" | "type">,
): OakRouter => {
